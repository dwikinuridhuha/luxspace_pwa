...
function numberCurrency(price) {
  const currentCurrency = Intl.NumberFormat("id-ID", {
    style: "currency",
    currency: "IDR",
  });

  return currentCurrency.format(price);
}
...
function ArrivedItem({ item }) {
  return (
    <div className="px-4 relative card group">
      ...
      <span className="">{numberCurrency(item.price)}</span>
